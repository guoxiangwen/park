...
import { Flex, Button, Modal, Icon, Popup, WingBlank, List, InputItem, Carousel } from 'antd-mobile';
...
export interface PlateinfoProps {
  ...
  discount: string;
  history:any;
  addCar: (plateRegion: string, plateNum: Array<string>, phone: string, plateRegionIndex: number, discount: string) =>void;
}
...
class Plateinfo extends React.Component<PlateinfoProps, PlateinfoState> {
  ...
  componentDidMount() {
    ...
    const isWxs = isWeiXin();
      ...
      if(isWxs){
        if(localStorage.getItem("wxopenid") == null || localStorage.getItem("wxopenid") == "undefined"){
          API.getOpenId({
            code: wxCode
          }).then((res:any) => {
  ...
  chooseCPNk() {
    Popup.show(<div className="popup">
      <div className="title">
        请选择车牌所属省份
        <span
          className="close"
      ...
      <ul className="cpnlist">
        {['京', '津', '冀', '晋', '蒙', '辽', '吉', '黑', '沪', '苏', '浙', '皖', '闽', '赣', '鲁', '豫', '鄂', '湘', '粤', '桂', '琼', '渝', '川', '贵', '云', '藏', '陕', '甘', '青', '宁', '港', '澳'].map((i, index) => (
          <li key={index} className={this.state.plateRegionIndex === index ? 'item active' : 'item'} onClick={() => { this.selected(index, i); }} >{i}</li>
        ))}
      </ul>
  ...
  addPlate() {
    ...
    if (this.state.discount) {
      API.checkCouponResource({
        ...
        if (res.data.code === 200) {
          this.props.addCar(this.state.plateRegion, this.state.plateNum, this.state.phone, this.state.plateRegionIndex, this.state.discount);
          if(terminal) {
            this.props.history.push(`/pay?terminal=${terminal}&couponCash=${res.data.data.value}`);
          } else {
  ...
  render() {
    ...
    const isWxs = isWeiXin();
      ...
      {(isWxs && wxCode || !isWxs) ? <div>
          ...
          selectedIndex={1}
        >
          {this.state.carouselData.length > 0 && this.state.carouselData.map((item: any) => <a key={item.id} href={item.href} style={hProp}>
            <img
              src={item.src}
              alt=""
              onLoad={() => {
          ...
          <FlexItem className="plate">
            <div className="info">
              <div className="region" onClick={this.chooseCPNk.bind(this)}>
                ...
                <span className="down"><Icon type="down" /></span>
                <span className="separator">|</span>
              </div>
              <div className="inputContainer">
                <input type="text" style={{ letterSpacing: '0.34rem' }} value={this.state.cpn} onChange={this.handlePlateNumChange.bind(this)} />
                <div className="cpnContainer">
            <div className={this.state.plateNum[0] != null ? 'show' : 'item'}>{this.state.plateNum[0]}
            </div>
            <div className={this.state.plateNum[1] != null ? 'show' : 'item'}>{this.state.plateNum[1]}
            </div>
            <div className={this.state.plateNum[2] != null ? 'show' : 'item'}>{this.state.plateNum[2]}
            </div>
            <div className={this.state.plateNum[3] != null ? 'show' : 'item'}>{this.state.plateNum[3]}
            </div>
            <div className={this.state.plateNum[4] != null ? 'show' : 'item'}>{this.state.plateNum[4]}
            </div>
            <div className={this.state.plateNum[5] != null ? 'show' : 'item'}>{this.state.plateNum[5]}
            </div>
                </div>
              </div>
            </div>
          ...
          <InputItem
            placeholder='请输入优惠券码'
            onChange={this.handleDiscountChange.bind(this)} value={this.state.discount}
          >
            优惠券
          </InputItem>
        </List>
        ...
        <InputItem
          placeholder="请输入手机号"
          onChange={this.handlePhoneChange.bind(this)} value={this.state.phone}
        >
          手机号
        </InputItem>
	      </List>
	      <WingBlank style={{marginBottom: 20}}>
		      <Button type="primary" disabled={this.state.plateNum.length !== 6} className="confirm" onClick={this.addPlate.bind(this)}>下一步</Button>
	      </WingBlank>
        </div>:null}
	    </div>);
  }
...
function mapDispatchToProp(dispatch) {
  return {
    addCar: (plateRegion, plateNum, phone, plateRegionIndex, discount) => {
      dispatch(addCar(plateRegion, plateNum, phone, plateRegionIndex, discount));
    }
  };
}
